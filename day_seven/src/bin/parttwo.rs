...
impl PartialOrd for Card {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        match (self, other) {
            ...
            (&Self::Number(_), &Self::Joker) => Some(std::cmp::Ordering::Greater),
            (&Self::Joker, &Self::Number(_)) => Some(std::cmp::Ordering::Less),
            (&Self::Joker, &Self::Joker) => Some(std::cmp::Ordering::Equal),
        }
    }
}
...
impl Hand {
    ...
    fn get_same_ones(card: Vec<Card>) -> (Vec<Card>, i32) {
        ...
        for c in card.iter() {
            ...
            } else if card.iter().filter(|x| *x == c).count() > 1 {
        ...
        if distinct == 0 && same_ones.contains(&Card::Joker) {
            if same_ones.len() == 5 {
                return (same_ones, distinct);
            }

            distinct = 1;
            same_ones.push(Card::Joker);
        }
